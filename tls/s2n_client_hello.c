...
int s2n_client_hello_send(struct s2n_connection *conn)
{
    ...
    S2N_BLOB_FROM_DATA(b, conn->secure.client_random, S2N_TLS_RANDOM_DATA_LEN);
    /* Create the client random data */
    GUARD(s2n_stuffer_init(&client_random, &b));

    S2N_BLOB_FROM_DATA(r, s2n_stuffer_raw_write(&client_random, S2N_TLS_RANDOM_DATA_LEN), S2N_TLS_RANDOM_DATA_LEN);
    ...
    if (conn->session_id_len == 0 && conn->config->use_tickets) {
        S2N_BLOB_FROM_DATA(session_id, conn->session_id, S2N_TLS_SESSION_ID_MAX_LEN);
