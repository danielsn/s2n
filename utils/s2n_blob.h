...
#define S2N_STACK_BLOB(name, requested_size)				\
    size_t name ## _requested_size = (requested_size);			\
    uint8_t name ## _buf[_requested_size] = {0};			\
...
#define S2N_BLOB_FROM_DATA(name, array, size)				\
    struct s2n_blob name = {0};						\
    GUARD(s2n_blob_init(&name, array, size))

#define S2N_BLOB_FROM_DATA_PTR(name, array, size)			\
    struct s2n_blob name = {0};						\
    GUARD_PTR(s2n_blob_init(&name, array, size))
